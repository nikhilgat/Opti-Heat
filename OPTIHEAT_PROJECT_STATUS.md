...
1. **Load-shifting does not reliably beat the market mean, and the reason is now well understood.** ⚠ [Certain] **THE CENTRAL FINDING OF THE 2026-08-19 SESSION.** Three genuine bugs were found and fixed first (below), then the controller was short-run tested (1-2 day windows, per the fast-iteration workflow -- see §3) across three distinct weather regimes to characterise where the price mechanism actually lands:
   ...
   **What would actually close the remaining gap** (follow-up work, not a tuning question): (a) a genuinely time-varying / gain-scheduled prediction model that schedules `Qmax_W` across the horizon from the previewed `T_air_C` trajectory instead of freezing it at step 0 -- a real MPC redesign, not available as a simple Adaptive MPC block option; (b) exposing time-varying OV bounds on the Simulink block so the tank's minimum can be raised specifically ahead of a known cutoff -- the auto-generated MPC block's mask does not currently expose time-varying constraints as an inport for this, so wiring it in is genuine Simulink structural surgery, not an `init_adaptive_mpc.m` change; or **(c) — user-suggested 2026-08-20, likely the simplest of the three and worth trying FIRST:** replace the instantaneous per-sample Heizgrenze gate (`T_air_C < 15°C`, re-evaluated every 15 min in `heating_curve.m`) with a **daily-mean or calendar heating-season gate**. The 2025-03-20 failure window has a daily-mean T_air_C around 9°C (safely inside the heating season) but instantaneous afternoon readings briefly cross 15°C -- that instantaneous crossing is what creates the within-day capacity cliff the frozen-horizon MPC can't see coming (item above). A smoothed/seasonal gate would likely keep that whole day in heating mode, removing the cliff at its source rather than working around it, AND is arguably more physically realistic (real heating-plant controls don't toggle a whole building's heat off for a few afternoon hours on momentary outdoor temperature -- they use exactly this kind of smoothed criterion). Would need: (i) decide daily-mean-rolling-window vs. fixed calendar dates, (ii) update `heating_curve.m`'s `HEATING_LIMIT_C` check for both the real plant (`hp_heater_physics.m` path) and the MPC's own internal model (`getAdaptivePlant.m`, the 2026-08-19 fix), (iii) re-run the same three-regime short tests to confirm the shoulder-season case improves without regressing the other two. Does NOT address the modulation-floor short-cycling case (2025-11-01) -- that's a separate, genuine hardware constraint, unrelated to the Heizgrenze.
...
1. **Close the Heizgrenze-foresight gap (§5 item 1).** Try **(c) first** — replace the instantaneous per-sample Heizgrenze gate with a daily-mean or calendar heating-season gate; likely the simplest fix and removes the within-day capacity cliff at its source rather than working around it. If that isn't enough on its own, (a) a genuinely time-varying/gain-scheduled prediction model that schedules `Qmax_W` across the horizon instead of freezing it at step 0, or (b) exposing time-varying OV bounds on the Simulink block so the tank's minimum can be raised ahead of a known cutoff. This is the single biggest lever left on the actual research claim (paying below market). Do NOT re-attempt an MV-weight-discount approach without a different mechanism — it was tried at two strengths and traced to not work (§5 item 1, fix 4).
